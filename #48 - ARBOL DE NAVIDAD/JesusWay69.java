...
public class JesusWay69 {
    ...
    public static List<Integer> coordinates = new ArrayList<>();
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (option) {
                ...
                case "2" -> 
                    showTree(topStar(myTree, true));
                
                case "3" -> 
                    showTree(topStar(myTree, false));
                
                case "4" -> 
                    showTree(addBalls(myTree));
                
                case "5" -> 
                    showTree(removeBalls(myTree));
                
                case "6" -> 
                    showTree(addLights(myTree));
                
                case "7" -> 
                    showTree(turnOnLights(myTree, coordinates));
                
                case "8" ->
                    showTree(turnOffLigths(myTree));
    ...
    public static List<Integer> storageCoordinates(int x, int y) {
        coordinates.add(x);
        coordinates.add(y);
        return coordinates;
    }
    ...
    public static char[][] turnOnLights(char[][] tree, List<Integer> coordinates) {
        if (tree == null) {
            System.out.println("ERROR: Hay que crear un árbol antes de poder modificarlo (opción 1)");
            return null;
        }
